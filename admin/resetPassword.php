<?php
    ...
    if(isset($_GET['email']) && isset($_GET['token'])){
        ...
        $stmt=$conn->prepare("SELECT id FROM adminuser WHERE email=? AND token=? AND token<>'' AND tokenExpire>NOW()");
        ...
        if($result->num_rows>0){
            if(isset($_POST['submit'])){
                ...
                if($newpass==$cnewpass){
                    $stmt_u=$conn->prepare("UPDATE adminuser SET token='', password=? WHERE email=?");
